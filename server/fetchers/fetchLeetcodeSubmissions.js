    ...
    try {
        ...
        if (LEETCODE_SESSION_COOKIE) {
            ...
            try {
                ...
                // Step 1: Check common known structures
                if (Array.isArray(submissionData?.data?.submissionList?.submissions)) {
                    submissions = submissionData.data.submissionList.submissions;
                } else if (Array.isArray(submissionData?.submissionList?.submissions)) {
                    submissions = submissionData.submissionList.submissions;
                } else if (Array.isArray(submissionData?.data?.submissions)) {
                    submissions = submissionData.data.submissions;
                } else if (Array.isArray(submissionData?.submissions)) {
                    ...
                    // Step 2: Fallback — search for the first array in the object
                    console.warn("No direct array match found, scanning object for any array...");
                    const findArray = (obj) => {
                    ...
                    const foundArray = findArray(submissionData);
                    if (foundArray) {
                        console.log(`Found array at unknown path, using it. Length: ${foundArray.length}`);
                        submissions = foundArray;
                    } else {
                        console.warn("No submissions array found at all in API response");
                        submissions = [];
                    }
                }
